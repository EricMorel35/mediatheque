...
- `mediatheque-scan` appelle l'API TMDB réelle (voir "Configuration TMDB" ci-dessous pour la clé) et écrit dans la vraie base — ne pas lancer sans le vouloir vraiment.
...
- Connexion : `jdbc:mysql://localhost:3306/mediatheque`, utilisateur/mot de passe `mediatheque`/`mediatheque` (identifiants de dev locaux en dur dans `docker-compose.yml` et les `application.properties` — pas prévus pour être réutilisés ailleurs).
...
- **Clé API** : externalisée via la variable d'environnement `TMDB_API_KEY` (plus de clé en dur dans `tmdb.properties` depuis un audit sécurité — l'ancienne clé avait été exposée dans l'historique git et doit être considérée compromise). C'est le **jeton v4 "Read Access Token"** (Bearer, ~200+ caractères) qu'il faut, pas la clé v3 classique (32 caractères) — le code envoie `Authorization: Bearer ${TMDB_API_KEY}` via un intercepteur sur le `RestTemplate` partagé (`AppTMDBConfiguration.restTemplate()`), il ne construit plus d'URL avec `api_key=`. Récupérable sur https://www.themoviedb.org/settings/api, section « Clé API (v3 auth) » — non, section **API Read Access Token**.
- **Piège Windows fréquent** : `[Environment]::SetEnvironmentVariable("TMDB_API_KEY", "...", "User")` écrit bien la valeur dans le registre (`[Environment]::GetEnvironmentVariable(..., "User")` la retrouve), mais un nouveau process shell peut hériter d'un environnement figé plus ancien et ne PAS voir la variable via `$env:TMDB_API_KEY` (chaîne vide) — même souci que pour `Path` ailleurs dans ce document. Vérifier `$env:TMDB_API_KEY` (pas juste `[Environment]::GetEnvironmentVariable`) avant de lancer `mediatheque-scan`/`mediatheque-webapp` ; sinon le Bearer token envoyé est vide et TMDB répond `401 Invalid API key`, un message trompeusement identique à celui d'une vraie mauvaise clé.
- **URL des jaquettes** : `urlCover` dans `tmdb.properties` pointe vers `https://image.tmdb.org/t/p/w500` (domaine officiel, confirmé via `GET /3/configuration`). L'ancien `cf2.imgobject.com` (legacy, projet ouvert en 2018) ne résout plus du tout en DNS — toute jaquette générée avec ce domaine échoue silencieusement côté navigateur (`ERR_NAME_NOT_RESOLVED`), sans erreur serveur visible.
- **Ne pas dupliquer `urlCover`/`urlYoutube` dans un autre fichier `.properties`** chargé via `@PropertySource` : `mediatheque-persistence` en avait une copie orpheline (`persistence.properties`, jamais lue par aucun code de ce module) qui entrait en collision avec celle de `mediatheque-tmdb` selon l'ordre de scan Spring — cause d'un bug de jaquettes à double slash (`w500//xxx.jpg`) difficile à diagnostiquer car non déterministe. Supprimé ; `tmdb.properties` (`mediatheque-tmdb`) est la seule source pour ces deux clés.
- `MovieSearchWrapped.getURLPoster()` (résultats de recherche/scan) doit concaténer `urlCover` + `poster_path` comme `MovieWrapped.getURLPoster()` (détail film) — un des deux ne le faisait pas historiquement et stockait le chemin brut TMDB en base (jaquettes cassées uniquement en liste, pas en détail). Les deux DTO factories (`buildLightMovieDTO`/`buildFullMovieDTO`) doivent aussi penser à renseigner le champ `cover`/`urlCover` de leur DTO — `buildFullMovieDTO` avait cet oubli (`cover` toujours `null` sur `/movie/{id}`).
...
- Import de 20 vrais films Disney+ via `mediatheque-scan` (remplace les 100 films de test factices) et 4 corrections découvertes au passage, voir section "Configuration TMDB" pour le détail :
  1. Auth TMDB basculée de la clé v3 (`api_key=`) vers le jeton v4 Bearer, seul format que possédait l'utilisateur (`AppTMDBConfiguration` : intercepteur `RestTemplate` + `tmdb.properties` nettoyé des `api_key=` en URL).
  2. Domaine CDN des jaquettes `cf2.imgobject.com` (mort, ne résout plus en DNS) → `image.tmdb.org` (officiel, confirmé via `GET /3/configuration`).
  3. Collision de propriété `urlCover`/`urlYoutube` entre `mediatheque-tmdb/tmdb.properties` (utilisé) et `mediatheque-persistence/persistence.properties` (orphelin, jamais lu par ce module) — supprimé `persistence.properties` et son `@PropertySource` dans `AppDAOConfiguration`.
  4. `MovieSearchWrapped.getURLPoster()` ne préfixait pas `poster_path` avec `urlCover` (contrairement à `MovieWrapped`) et `MovieDTOFactoryImpl.buildFullMovieDTO` ne renseignait jamais `cover` — jaquettes cassées respectivement en liste et en détail film.
  5. `WALL-E` ne matchait pas sur une recherche TMDB standard (titre réel avec caractère spécial `WALL·E`) — remplacé par `Soul` dans le jeu de films importés plutôt que de bricoler une insertion manuelle.
